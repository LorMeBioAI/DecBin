...
class VAE(nn.Module):
    ...
    def encode(self, data_loader):
        ...
        index = np.empty((length),dtype=np.int)
    ...
    def trainepoch(self, data_loader, epoch, optimizer, batchsteps, logfile):
        ...
        logging.info(f'Epoch: {epoch + 1:4} Loss: {epoch_loss / (1+len(data_loader)):.6f}\tEC: {epoch_celoss / (1+len(data_loader)):.7f}\t'
                     f'EP: {epoch_mseloss / (1+len(data_loader)):.6f}\tKLD: {epoch_kldloss / (1+len(data_loader)):.4f}\
                     tBatchsize: {data_loader.batch_size}')
        self.writer.add_scalar('loss', epoch_loss, global_step=epoch)
        self.writer.add_scalar('kld_loss', epoch_kldloss, global_step=epoch)
        self.writer.add_scalar('mseloss', epoch_mseloss, global_step=epoch)
        self.writer.add_scalar('celoss', epoch_celoss, global_step=epoch)
    ...
    def trainmodel(self, dataloader, *, nepochs=500, lrate=1e-3,
        ...
        logging.basicConfig(filename=os.path.join(self.writer.log_dir, 'fine_training.log'), level=logging.DEBUG)
        ...
        self.save(f"{save_path}/model.pt")
...
def train_vae(output, latent_dims, hidden_layers, epochs, constraints, cuda):
    ...
    dloader = make_data_loader(cov_profiles, comp_profiles)
    vae.trainmodel(
        dloader, save_path=output, nepochs=epochs, batchsteps=[50, 100, 150])
