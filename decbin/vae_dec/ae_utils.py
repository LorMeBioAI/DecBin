...
json_path = os.path.join(os.path.dirname(__file__), "hyper_params.json")
with open(json_path, "r") as f:
    h_params = json.load(f)


def make_data_loader(covs, profs, *, batch_size=1024, drop_last=True, shuffle=True):
...
class VAE(nn.Module):
    def __init__(self, cov_size, prof_size, *, latent_dims=32, hidden_layers=[128, 128], constraints=None, device='cpu'):
    ...
    def encode(self, data_loader):
        ...
        index = np.empty((length), dtype=int)
        ...
        with torch.no_grad():
            for covs, profs, indices in data_loader:
                ...
                index[row: row + len(indices)] = indices
                row += len(mu)
    ...
    def trainepoch(self, data_loader, epoch, optimizer, batchsteps):
        ...
        for covs_in, profs_in, indices in data_loader:
        ...
        avg_loss = epoch_loss / (1 + len(data_loader))
        avg_ec = epoch_celoss / (1 + len(data_loader))
        avg_ep = epoch_mseloss / (1 + len(data_loader))
        avg_kld = epoch_kldloss / (1 + len(data_loader))
        #print(f'Epoch: {epoch + 1:4} Loss: {avg_loss:.6f}\tEC: {avg_ec:.7f}\tEP: {avg_ep:.6f}\tKLD: {avg_kld:.4f}\tBatchsize: {data_loader.batch_size}')
    ...
    def calc_loss(self, covs_in, covs_out, profs_in, profs_out, mu, logsigma, indices):
        ...
        if self.constraints is not None:
            ...
            if len(must_not_link_pairs) > 0:
                loss_mnl = torch.max(torch.tensor(0.0).to(self.device), 10 - (mu[must_not_link_pairs.T[0]] - mu[must_not_link_pairs.T[1]]).pow(2).sum(dim=1).mean())
    ...
    def trainmodel(self, dataloader, *, nepochs=500, lrate=1e-3, batchsteps=[25, 75, 150, 300], save_path=None):
        ...
        model_save = os.path.join(save_path, "model.pt")
...
def vae_encode(output, latent_dims, hidden_layers, epochs, constraints, cuda):
    ...
    comp_profiles = np.load(com_path)
    ...
    print(f"Model param count = {count_parameters(vae)}")
    print(vae)

    dloader = make_data_loader(cov_profiles, comp_profiles)
    vae.trainmodel(dloader, save_path=output, nepochs=epochs, batchsteps=[50, 100, 150])

    dloader_encode = make_data_loader(cov_profiles, comp_profiles, drop_last=False, shuffle=False)
    latent, index = vae.encode(dloader_encode)

    latent_out = os.path.join(output, "latent.npy")
    np.save(latent_out, latent)


def train_vae(output, latent_dims, hidden_layers, epochs, constraints, cuda):
    com_path = os.path.join(output, "profiles", "com_profs.npy")
    cov_path = os.path.join(output, "profiles", "cov_profs.npy")
    comp_profiles = np.load(com_path)
    cov_profiles = np.load(cov_path)
